...
export default function AdminCatalogPage () {
  ...
  const toggleStatus = async (cat: Catalog) => {
    ...
    if (!confirm(newStatus ? 'Aktifkan game ini?' : 'Matikan game ini?')) return
  ...
  const handleDelete = async (cat: Catalog) => {
    ...
    try {
      const res = await deleteApi(`/admin/catalogs/${cat.cardcode}`) // <-- Pakai deleteApi
      if (res.ok) {
        fetchCatalogs()
      } else {
        alert(await readResponseError(res, 'Gagal menghapus katalog.'))
      }
    } catch (error) {
      console.error(error)
      alert('Terjadi kesalahan sistem saat menghapus katalog.')
    }
  }
                      ...
                      <td className='py-4 px-2 text-right'>
                        <div className='flex items-center justify-end gap-2'>
                          <button
                            ...
                            className='text-slate-400 hover:text-[#e491c9] bg-white/[0.02] hover:bg-[#e491c9]/10 p-2 rounded-full transition-all duration-300 border border-white/[0.04] hover:border-[#e491c9]/30 hover:shadow-[0_0_10px_rgba(228,145,201,0.2)]'
                            title='Edit Catalog'
                          >
                            <svg
                          ...
                          <button
                            ...
                            className='text-slate-400 hover:text-rose-400 bg-white/[0.02] hover:bg-rose-500/10 p-2 rounded-full transition-all duration-300 border border-white/[0.04] hover:border-rose-500/30 hover:shadow-[0_0_10px_rgba(244,63,94,0.2)]'
                            title='Hapus Catalog'
                          >
                            <svg
                              xmlns='http://www.w3.org/2000/svg'
          ...
          >
            ...
            <div className='relative z-10'>
              <button
                ...
                className='absolute right-0 top-0 flex h-10 w-10 items-center justify-center rounded-full border border-white/[0.08] bg-white/[0.025] text-lg text-white/45 transition-colors hover:border-white/[0.16] hover:bg-white/[0.06] hover:text-white outline-none focus:outline-none'
              >
                <svg
                  aria-hidden='true'
                  viewBox='2 2 20 20'
              ...
              >
                ...
                {/* ========================================================
                    KONFIGURASI TARGET INPUT DINAMIS
                   ======================================================== */}
                <div className='rounded-2xl border border-purple-500/20 bg-purple-500/[0.03] p-4 sm:col-span-2 space-y-4'>
                  <p className='text-[11px] font-bold text-purple-300 uppercase tracking-widest flex items-center gap-2'>
                    <span>🎮</span> Konfigurasi Input Akun Player
                  </p>
                  ...
                  <div className='grid grid-cols-1 gap-4 sm:grid-cols-2'>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        placeholder='Contoh: User ID, UID, GrowID, Riot ID'
                      />
                    </div>
                    ...
                    {formData.target_type === 'SERVER_DROPDOWN' && (
                      <div>
                        ...
                        <input
                          type='text'
                          value={formData.target_server_options}
                          onChange={e =>
                            setFormData({
                              ...formData,
                              target_server_options: e.target.value
                            })
                          }
                          className='w-full bg-white/[0.02] border border-white/[0.05] rounded-xl p-3 text-white focus:border-purple-500/50 outline-none text-sm font-mono'
                          placeholder='Asia, America, Europe, TW_HK_MO'
                        />
                      </div>
                    )}
                  </div>
                ...
                <div className='grid grid-cols-1 gap-2 sm:col-span-2 sm:grid-cols-2 lg:grid-cols-4'>
                  <label className='flex cursor-pointer items-center gap-2 rounded-xl border border-white/[0.05] bg-white/[0.02] p-3 text-[10px] font-bold uppercase tracking-widest text-purple-300/70 hover:bg-white/[0.04] transition-colors'>
                    <input
                      type='checkbox'
