  ...
  return (
    ...
    >
      ...
      <span className="min-w-0 flex-1">
        ...
        {method.enabled ? (
          <>
            <span className="mt-1.5 block text-xs font-medium tabular-nums text-white/[0.68]">
              Total {formatPaymentAmount(method.total_amount)}
            </span>
            <span
...
export default function PaymentMethodSelector({
  ...
  const recommendations = useMemo(() => {
    const rankedMethods = availableMethods
      ...
      .sort(
        (first, second) =>
          (first.recommendation_rank || 0) - (second.recommendation_rank || 0),
      );
  ...
  return (
    ...
    >
      ...
      {hasSelectedProduct && !loading && methods.length > 0 && (
        <div className="mt-8">
